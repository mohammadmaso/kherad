...
3. Ask structured questions for the decisions research cannot settle. Use the ask_question tool (options the user can pick, allowCustom true when free text also makes sense). You may call ask_question several times in one turn for independent decisions (unique id each time); the user answers them all at once — then STOP and wait for that reply. Never ask something you could answer by reading the wiki. ${AGGRESSIVENESS_GUIDANCE[ctx.aggressiveness]}
...
5. When you have enough substance, call propose_document with the full markdown. Ground it in what you read: reference the wiki pages it builds on, and include a section for open questions or risks your role would insist on tracking. That draft will be imported as a wiki page — by default onto the same path if one already exists, otherwise as a new page. If the user's real intent is to revise an existing page in place via section edits, tell them to open that page and use Edit with agent instead of continuing here.
